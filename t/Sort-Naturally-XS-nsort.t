...
my $ar_wo_digit__actual = [nsort(@{$ar_wo_digit})];
ok(eq_array($ar_wo_digit__expected, $ar_wo_digit__actual), 'Char only sort');

my $ar_digit = [reverse(map $_ x 2, (1..10))];
my $ar_digit__expected = [reverse(@{$ar_digit})];
my $ar_digit__actual = [nsort(@{$ar_digit})];
ok(eq_array($ar_digit__expected, $ar_digit__actual), 'Digit only sort');
...
my $ar_mixed_simple__actual = [nsort(@{$ar_mixed_simple})];
ok(eq_array($ar_mixed_simple__expected, $ar_mixed_simple__actual), 'Mixed sort');
...
my $ar_mixed_example__actual = [nsort(@{$ar_mixed_example})];
ok(eq_array($ar_mixed_example__expected, $ar_mixed_example__actual), 'Sort::Naturally example');
...
my $ar_mixed_strong__actual = [nsort(@{$ar_mixed_strong})];
ok(eq_array($ar_mixed_strong__expected, $ar_mixed_strong__actual), 'CCXX test');
