...
ok(eq_array($ar_wo_digit__expected, [sort ncmp @{$ar_wo_digit}]));


my $ar_digit = [reverse(map $_ x 2, (1..10))];
my $ar_digit__expected = [reverse(@{$ar_digit})];
ok(eq_array($ar_digit__expected, [sort ncmp @{$ar_digit}]));
...
ok(eq_array($ar_mixed_simple__expected, [sort ncmp @{$ar_mixed_simple}]));
...
ok(eq_array($ar_mixed_example_expected, [sort {ncmp($a, $b)} @{$ar_mixed_example}]));
...
ok(eq_array($ar_mixed_strong__expected, [sort ncmp @{$ar_mixed_strong}]));
